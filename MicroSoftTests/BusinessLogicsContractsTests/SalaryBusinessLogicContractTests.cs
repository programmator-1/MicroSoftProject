...
internal class SalaryBusinessLogicContractTests
{
    ...
    public void CalculateSalaryByMounth_CalculateSalary_Test()
    {
        ...
        _installStorageContract.Setup(x => x.GetList(It.IsAny<DateTime>(), It.IsAny<DateTime>(), It.IsAny<string>(), It.IsAny<string>())).Returns([new InstallDataModel(Guid.NewGuid().ToString(), Guid.NewGuid().ToString(), workerId, 100, 10)]);
        ...
        _workerStorageContract.Setup(x => x.GetList(It.IsAny<bool>(), It.IsAny<string?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>())).Returns([new WorkerDataModel(workerId, "Test", Guid.NewGuid().ToString(), DateTime.UtcNow, DateTime.UtcNow, "test.test.com", false)]);
        var sum = 0.0;
        var expectedSum = postSalary + saleSum * 0.1;
        _salaryStorageContract.Setup(x => x.AddElement(It.IsAny<SalaryDataModel>())).Callback((SalaryDataModel x) => {
            sum = x.Salary;
        });
        //Act 
    ...
    public void CalculateSalaryByMounth_WithSeveralWorkers_Test()
    {
        ...
        var list = new List<WorkerDataModel>() {
            new(worker1Id, "Test", Guid.NewGuid().ToString(), DateTime.UtcNow, DateTime.UtcNow, "test.test.com", false),
            new(worker2Id, "Test", Guid.NewGuid().ToString(), DateTime.UtcNow, DateTime.UtcNow, "test.test.com", false),
            new(worker3Id, "Test", Guid.NewGuid().ToString(), DateTime.UtcNow, DateTime.UtcNow, "test.test.com", false)
        };
        _installStorageContract.Setup(x => x.GetList(It.IsAny<DateTime>(), It.IsAny<DateTime>(), It.IsAny<string>(), It.IsAny<string>()))
            .Returns([
                new InstallDataModel(Guid.NewGuid().ToString(), Guid.NewGuid().ToString(), worker1Id, 100, 10), 
                new InstallDataModel(Guid.NewGuid().ToString(),Guid.NewGuid().ToString(), worker1Id, 100, 10), 
                new InstallDataModel(Guid.NewGuid().ToString(),Guid.NewGuid().ToString(), worker2Id, 100, 10), 
                new InstallDataModel(Guid.NewGuid().ToString(),Guid.NewGuid().ToString(), worker3Id, 100, 10), 
                new InstallDataModel(Guid.NewGuid().ToString(),Guid.NewGuid().ToString(), worker3Id, 100, 10)]);
    ...
    public void CalculateSalaryByMounth_WithoitSalesByWorker_Test()
    {
        ...
        _workerStorageContract.Setup(x => x.GetList(It.IsAny<bool>(), It.IsAny<string?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>()))
            .Returns([new WorkerDataModel(workerId, "Test", Guid.NewGuid().ToString(), DateTime.UtcNow, DateTime.UtcNow, "test@test.com", false)]);
        var sum = 0.0;
    ...
    public void CalculateSalaryByMounth_PostStorageReturnNull_ThrowException_Test()
    {
        ...
        var workerId = Guid.NewGuid().ToString();
        _installStorageContract.Setup(x => x.GetList(It.IsAny<DateTime>(), It.IsAny<DateTime>(), It.IsAny<string>(), It.IsAny<string>()))
            .Returns([new InstallDataModel(Guid.NewGuid().ToString(),Guid.NewGuid().ToString(), workerId, 100, 10)]);
        _workerStorageContract.Setup(x => x.GetList(It.IsAny<bool>(), It.IsAny<string?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>(), It.IsAny<DateTime?>()))
            .Returns([new WorkerDataModel(workerId, "Test", Guid.NewGuid().ToString(), DateTime.UtcNow, DateTime.UtcNow, "test.test.com", false)]);
        //Act&Assert 
        Assert.That(() => _salaryBusinessLogicContract.CalculateSalaryByMounth(DateTime.UtcNow), Throws.TypeOf<NullListException>());
